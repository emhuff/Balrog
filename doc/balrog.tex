...
\draw node [com, right=3.2cm of truth, align=center, text width=2.5cm] (parse) {\textbf{\hyperref[sec:pyconfig]{Parse simulation configuration}}};
...
\draw[->](truth) -| node {} (rm1);
\draw[->](truth) -- node {} (rm2);
\draw[->](imaging) -- ($(imaging.east) + (10.3,0)$) |- node {} (draw);
...
Diamonds are decision points.
Blue text is hyperlinked to \galsim{} documentation.
