...
Accordingly, one is able to answer the question of how the measured
...
\sex\footnote{\url{https://www.astromatic.net/software/sextractor}}.
\balrog{} facilitates the ease of running these codes en masse over
many images, filling in many of the bookkeeping steps in an automated way.
...
An often overlooked feature of scientific software is its usability.
\balrog{} has been written making our best attempts at user-friendliness.
Example files have been packaged with the code so that following installation,
the pipeline is able to run out of the box.
Users can run over these example files while they become familiar with
the \balrog{} environment.
To preserve an intuitive feel, \balrog{}'s simulation framework mimics ordinary \py{} syntax. 
Files and directories are given understandable names. Log files are automatically written,
useful in cases where something does go wrong and debugging must occur.
...
\autoref{sec:motivation} addresses just that.
...
\autoref{sec:detailed} builds upon \autoref{sec:quick} spelling out all the usage details,
including commad line arguments and the framework for specifying galaxy simulations.
The format of \balrog{}'s outputs are explained and some debugging hints are offered.
...
\balrog{} operates using two initally unrelated pieces of information: real imaging data and simulated galaxies.
The central idea of \balrog{} is to place the simulated galaxies into the images themselves.
...
\balrog{} begins by reading in an astronomical image of pixellated flux values.
Required with each image is an associated weight map and PSF model. 
The only file type \balrog{} accepts for these three inputs is FITS; 
any other file type will trigger an error.
Furthermore, \balrog{} requires the PSF model be given in the format generated by 
\psfex{}\footnote{\url{www.astromatic.net/software/psfex}}.
...
\balrog{} simulations will be done in WCS coordinates as opposed to image coordinates, and hence
\balrog{} requires each image contain a WCS solution. \balrog{} reads this solution from the image's header.
If no WCS exists in the header, a constant default pixel scale of 0.263 arcsec/pixel is adopted.
...
Conceptually, the bulk of the undertaking goes into simulationg the galaxies, which is described
...
\subsection{Galaxy Simulation}
\label{sec:galsim}

To simulate galaxies, \balrog{} makes use of \galsim{}. 
It calls the \texttt{Sersic} class to define the galaxies' light distriubtions as \sersic{} profiles. By adding
\texttt{Sersic} objects together, \balrog{} allows galaxies to be composed of as many superimposed
...
away from the $x$-direction. In addtion to its \sersic{} components, each galaxy
...
\subsubsection{Postage Stamp Size}
...
The comprehensive usage instructions are saved for \autoref{sec:detailed}.
The following start up sections will refer readers to the relevant sections of \autoref{sec:detailed} for more details.
...
The default example's image data and PSF live in \texttt{\$INSTALLDIR/default\_example/}.
...
If no \opt{outdir} is given, it defaults to \texttt{\$INSTALLDIR/default\_example/output}.
