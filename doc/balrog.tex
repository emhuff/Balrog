...
within the file specified by this command line argument.
...
\texttt{\$INSTALLDIR/config.py} builds galaxies as single component \sersic{} objects.
The core functions defined in \pyconfig{} have a strictly
...
many users will likely be able to extrapolate directly from the examples without reading lengthy documentation. 
Furthermore, the file includes descriptive comment lines as a guide, explaining what the different functions are used to do.
...
\texttt{balrog\_image} includes a copy of an image \balrog{} either used or generated.
Ones with \texttt{.sim} include the simulated galaxies.
...
It may seem that a single run produce a substantial amount of output, but the idea
...
Rather, we simply offer a few comments to help run \sex{} successfully.
