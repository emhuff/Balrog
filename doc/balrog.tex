...
\usepackage{amsfonts,amsmath,amssymb,gensymb,soul,graphicx,titlesec,tabu,longtable,array,natbib,listings}
...
\lstset{aboveskip=15pt,belowskip=15pt,showstringspaces=false,basicstyle=\ttfamily,columns=fullflexible,escapeinside={*}{*}}
...
\codett{runbalrog} & \codett{\$INSTALLDIR/balrog.py} \\
\end{tabular}
\end{table}
...
Thus, we attempt to build an enviroment which makes it easy for users to fold in additional content themselves.
...
We have included some convenience features to ease the simulation specfications,
...
In the remainder of \autoref{sec:pyconfig} we concentrate on \pyconfig{}'s usage, with a focus on elucidating its syntax.
\autoref{sec:user} explains how custom command line arguments are added to \balrog{} using \pyconfig{}.
\autoref{sec:simrules} illustrates how \pyconfig{} specifies the process of generating simulated galaxy properties and images.
\autoref{sec:sexoverride} describes how \pyconfig{} can be used to override \sex{} settings in \opt{sexconfig}.
As an aid to the text, while reading this section, 
...
def CustomArgs(parser):
    parser.add_argument( "-cs", "--catalogsample", help="Catalog used 
                            to sample simulated galaxy parameter 
                            distriubtions from",  
...
and saved them into an objects called \parseargs{}.
...
Changes within these functions to  \parseargs{} attributes, both those derived from native or custom arguments,
are also effectively \emph{local only}.
Modification made in one of the functions do not propogate outside that function.
It has been written this way so users do not need to consider the order in which the three functions execute
...
\noindent \simrules{} has 11 attributes in total, whose names are intended to be simple to understand.
...
\codett{rules.x} & Galaxy centroid $x$-coordinate [pixels], first pixel = 1\\
\codett{rules.y} & Galaxy centroid $y$-coordinate [pixels], first pixel = 1\\
\codett{rules.g1} & Reduced shear, $g_1$ component \\
\codett{rules.g2} & Reduded shear, $g_2$ component \\
\codett{rules.magnification} & Magnification, $1 + \kappa$ \\
\codett{rules.nProfiles} & Number of superimposed \sersic{} profiles \\
\codett{rules.sersicindex} & \sersic{} index \\
\codett{rules.halflightradus} & \sersic{} half light radius \\
\codett{rules.magnitude} & Galaxy brightness in magnitudes \\
\codett{rules.axisratio} & Minor to major axis ratio, $b/a$ \\
\codett{rules.beta} & Orientaiton angle of major axis (measured from $x$-axis)
\end{longtabu}

\noindent \codett{rules.sersicindex}, \codett{rules.halflightradius}, \codett{rules.magnitude}, \codett{rules.axisratio},
and \codett{rules.beta} must be arrays, whose length is equal to \codett{rules.nProfiles}. For example, simulating
...
Finally a function can be used. Users write their own \codett{python} function,
then feed this function and its necessary arguments as the arguments to
\balrog{}'s \codett{Function} command. The defined function must return an
...
\codett{Function} affords both flexiblity and convenience when deciding
how to sample the simulated galaxies. See \config{} for examples
using \codett{Function} as well as other sampling types.
...
Constant & \multicolumn{2}{l}{\codett{rules.g1 = 0}} \\
Array & \codett{rules.axisratio = [} & \codett{np.ones(args.ngal)/np.arange(1,args.ngal+1), sampled.axisratio[0]]} \\
Catalog & \codett{rules.magnitude = [} & \codett{Catalog(`cosmos.fits', 0, `IMAG'), Catalog(`cosmos.fits', 0, `IMAG')]} \\
Function & \multicolumn{2}{l}{\codett{rules.g2 = Function(function=NFW2, args=(sampled.x,sampled.y))}}
\end{longtabu}
...
the first element of \codett{rules.axisratio} is an array decreasing incrementally from 1 to 0.
The second elemment of \codett{rules.axisratio} then says to use whatever the sampled values of
 the first element turn out to be. Here, this equates to setting the second element of \codett{rules.axisratio}
 to the same array as the first element of \codett{rules.axisratio}, so the use of \simsamp{} is not 
 really necessary. However, return to the fourth example in \autoref{tab:simtype}. Image \codett{NFW2}
 as a function which takes two arguments, \codett{x} and \codett{y}, which returns the $g_2$ component
 of shear at position ($x$,$y$) from an NFW halo with mass $10^{15} M_{\odot}$ whose center lies at (0,0).
 Now image \codett{sampled.x} and \codett{sampled.y} were sampled randomly:
 
 \setlength{\tabcolsep}{0pt}
\begin{longtabu} to \linewidth {X}
\codett{def Random(minimum, maximum, size):} \\
\hspace{20pt}  \codett{return np.random.uniform(minimum, maximum, size)}\\
\\
\codett{rules.x = Random(args.xmin, args.xmax, args.ngal)} \\
\codett{rules.y = Random(args.ymin, args.ymax, args.ngal)}
\end{longtabu}
\setlength{\tabcolsep}{6pt}
\addtocounter{table}{-1}
 
\noindent \codett{sampled.x} and \codett{sampled.y} now represent the values
