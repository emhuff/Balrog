...
In addition, \balrog{} supports a mechanism for users to define their own command line arguments.
This section is subdivided into two segments, one for each of the categories.
\autoref{sec:builtin} enumerates all the available options built-in to \balrog{},
and \autoref{sec:user} explains how to add custom ones on top of those.
To print a summary of \balrog{}'s command line environment, 
including what has been added by the user, run the following command.
\inline
\bcmd{\opt{help}}
\inline
This prints the names of each parameter, along with available help strings.
The help strings are intended to be a useful quick reference tool,
...
Accordingly, we include a more comprehensive complement within this section.


\section{Built-In Command Line Environment}
\label{sec:builtin}

Without further ado, let us direct the attention to to \autoref{tab:opts}.
It is a fairly sizeable table containing an entry for each of \balrog{}'s command line arguments.
...
Warnings or errors are reaised as needed.
\autoref{tab:opts} has been group into subcategories of similar parameters.
