...
\usepackage{amsfonts,amsmath,amssymb,gensymb,soul,graphicx,titlesec,tabu,longtable,array,natbib,listings,booktabs,hhline}
...
\sersic{} index, half light radius, brightness magnitude, minor to major axis ratio, 
...
\rules{beta} & Orientaiton angle of major axis (measured from $x$-axis) [degrees] \\ \hline
\end{tabular}
\end{table}
...
Catalog & \codett{rules.magnitude = Catalog(file=`cosmos.fits', ext=0, col=`IMAG')} \\
Function & \codett{rules.y = Function(function=rand,} \\
 & \hspace{1.55in} \codett{args=[args.xmin,args.xmax,args.ngal])} \\ \hline
\end{tabular}
\end{table}
...
\codett{rules.magnitude = Catalog(file=`cosmos.fits', ext=0, col=`IMAG')}.
...
\simsamp{} can also be used as an argument within the \codett{args} passed to a sampling function,
...
\rules{sersicindex}, \rules{halflightradius}, \rules{magnitude}, \rules{axisratio},
and rules{beta} must be arrays, whose length is equal to rules{nProfiles}. For example, simulating
...
the first element of \rules{axisratio} is an array decreasing incrementally from 1 to 0.
The second elemment of \rules{axisratio} then says to use whatever the sampled values of
 the first element turn out to be. Here, this equates to setting the second element of rules{axisratio}
 to the same array as the first element of \rules{axisratio}, so the use of \simsamp{} is not 
