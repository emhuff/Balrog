...
First, \balrog{}'s required input data is discussed.
Next, the process of simulating galaxies is described.
Finally, \balrog{}'s image processing steps are laid out.
As a complement to the text we have included \autoref{fig:flowchart},
a flowchart visually representing the pipeline's flow.
Roughly speaking, the text steps throgh the flowchart from left to right.
However, \balrog{} runs trace out more than a single horizontal line through the diagram.
...
These start up pointers below will refer readers to the relevant comprehensive sections for more details.
